...
function ccms_preprocess_maintenance_page(&$variables) {
  ...
  drupal_add_css(drupal_get_path('theme', 'ccms') . '/css/maintenance-page.css');
}
...
function ccms_preprocess_node(&$variables) {
  ...
  // DJS Adds Javascript to node types with inline labels where the document format dictates the
  // label fields need to be the same height as the field content.
  // Unfortunately WKHTMLTOPDF doesn't like the inline style="height: ???px" that jQuery inserts
  // so I've had to remove this
  /* if (in_array($variables['type'], array('validation_documentation_set', 'module_version'))) {
    drupal_add_js(drupal_get_path('module', 'ccms') . '/js/set-inline-label-height.js');
  } */
}
